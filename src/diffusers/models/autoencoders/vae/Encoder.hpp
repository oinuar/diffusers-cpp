...
public:
    ...
    Tensor forward(Scope scope, Tensor sample) {
        auto conv_in = std::static_pointer_cast<Conv2d>(modules["conv_in"]);
        sample = conv_in->forward(scope, sample);
