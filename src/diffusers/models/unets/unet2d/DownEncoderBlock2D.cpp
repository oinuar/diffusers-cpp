...
Tensor DownEncoderBlock2D::forward(Runtime& runtime, Tensor hidden_states) {
    ...
    if (add_downsample_) {
        ...
        for (auto i = 0; i < downsamplers->size(); ++i)
            hidden_states = std::static_pointer_cast<Downsample2D>((*downsamplers)[i])->forward(runtime, hidden_states);
    }
