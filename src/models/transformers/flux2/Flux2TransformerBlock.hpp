...
public:
    ...
    std::tuple<Tensor, Tensor> forward(
        ...
        auto norm_encoder_hidden_states = norm1_context->forward(ctx, encoder_hidden_states);
        ...
        norm_hidden_states = norm2->forward(ctx, hidden_states);
        ...
        auto ff_output = ff->forward(ctx, norm_hidden_states);
        ...
        norm_encoder_hidden_states = norm2_context->forward(ctx, encoder_hidden_states);
        ...
        auto context_ff_output = ff_context->forward(ctx, norm_encoder_hidden_states);
