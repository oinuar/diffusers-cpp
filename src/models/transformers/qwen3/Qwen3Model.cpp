Qwen3Model::Qwen3Model(const Qwen3Config& config) : num_hidden_layers_(config.num_hidden_layers) {
    ...
    for (auto i = 0; i < config.num_hidden_layers)
    ...
    modules["rotary_emb"] = std::makea_shared<Qwen3RotaryEmbedding>(config);
...
{
    if (use_cache)
        throw std::runtime_error("use_cache = True is not implemented");

    if (inputs_embeds)
        inputs_embeds = inputs_embeds.value().to(GGML_TYPE_F32); // FloatTensor
    ...
    for (auto i = 0; i < num_hidden_layers_) {
