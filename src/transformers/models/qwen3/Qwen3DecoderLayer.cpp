...
Qwen3DecoderLayer::Qwen3DecoderLayer(const Qwen3Config& config, int layer_idx) {        
    modules["self_attn"] = std::make_shared<Qwen3Attention<ScaledDotProductAttention<FlashAttentionOp>>>(config, layer_idx);
...
Tensor Qwen3DecoderLayer::forward(
    ...
    hidden_states = self_attn->forward(runtime, hidden_states, position_ids, attention_mask, past_key_values);
