from utils import TestCase
import torch

class TestTensorAdd(TestCase):
    def test_add_same_shape(self):
        a = torch.randn(2, 8)
        b = torch.randn(2, 8)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_trailing_dimension_broadcast(self):
        # [2, 4, 8] + [8] -> [2, 4, 8]
        a = torch.randn(2, 4, 8)
        b = torch.randn(8)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_singleton_dimension_broadcast(self):
        # [2, 1, 8] + [2, 77, 8] -> [2, 77, 8]
        a = torch.randn(2, 1, 8)
        b = torch.randn(2, 77, 8)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_both_tensors_expand(self):
        # [2, 1, 8] + [1, 77, 1] -> [2, 77, 8]
        a = torch.randn(2, 1, 8)
        b = torch.randn(1, 77, 1)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_lhs_broadcast(self):
        # [8] + [2, 4, 8] -> [2, 4, 8] — lhs is the broadcastable operand.
        a = torch.randn(8)
        b = torch.randn(2, 4, 8)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_lhs_singleton_broadcast(self):
        # [1, 77, 1] + [2, 77, 8] -> [2, 77, 8] — lhs is the broadcastable operand.
        a = torch.randn(1, 77, 1)
        b = torch.randn(2, 77, 8)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_scalar(self):
        a = torch.randn(2, 3, 4)
        b = torch.tensor(2.0)

        expected = a + b

        actual = self.cli(
            'add_scalar',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_scalar_add(self):
        a = torch.tensor(2.0)
        b = torch.randn(2, 3, 4)

        expected = a + b

        actual = self.cli(
            'scalar_add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_cross_rank_broadcast(self):
        # [2, 1, 8] + [77, 1] -> [2, 77, 8] — neither operand is a
        # broadcast superset of the other (cross-rank).
        a = torch.randn(2, 1, 8)
        b = torch.randn(77, 1)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])

    def test_add_4d_cross_broadcast(self):
        # [1, 2, 1, 3] + [4, 1, 5, 1] -> [4, 2, 5, 3] — neither operand is
        # a broadcast superset of the other (4D).
        a = torch.randn(1, 2, 1, 3)
        b = torch.randn(4, 1, 5, 1)

        expected = a + b

        actual = self.cli(
            'add',
            '--lhs', str(a.tolist()),
            '--rhs', str(b.tolist()),
        )

        self.assertTensors(actual, [expected])
